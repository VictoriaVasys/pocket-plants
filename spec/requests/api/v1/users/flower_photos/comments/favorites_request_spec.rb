...
describe 'flower photo favorites' do
  ...
  it "can create new favorite" do
    post "/api/v1/users/flower_photos/#{flower_photo.id}/comments/#{comment_1.id}/favorites"
  ...
  it "can delete exisiting favorite" do
    expect(comment_1.favorites.count).to eq(3)
    
    delete "/api/v1/users/flower_photos/#{flower_photo.id}/comments/#{comment_1.id}/favorites/#{favorite_1.id}"
    
    favorite_message = JSON.parse(response.body)
    
    expect(response).to have_http_status(202)
    expect(favorite_message["favorites_count"]).to eq(2)
    expect(comment_1.favorites.count).to eq(2)
  end
end