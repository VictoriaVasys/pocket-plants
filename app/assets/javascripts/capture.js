...
(function() {
  ...
  function postToGvisionAPI(data) {
    ...
    // const google_key = Figaro.env.google_application_credentials
    debugger
    return $.ajax({
      type: "POST",
      url: 'https://vision.googleapis.com/v1/images:annotate?key=' + ENV['GOOGLE_APPLICATION_CREDENTIALS'],
      body: JSON.stringify(gvisionData),
      success: function(gvisionLabels) {
        filterLabels(gvisionLabels)
      },
    }).then(function(filteredDescriptions) {
      plantFamily(filteredDescriptions)
    }).then(function(plantFamily){
      postToHome(plantFamily)
    })
  }
